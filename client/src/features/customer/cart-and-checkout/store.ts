...
const waitForRazorpay = (timeOut = 4000): Promise<void> => {
...
const getGuestResponse = (): CustomerCartResponse => {
...
const getGuestSyncPayload = (): SyncCustomerCartBody => {
  return {
...
const addGuestItem = (item: Omit<GuestCartItem, "quantity">) => {
  const items = readGuestItems();
...
const increaseGuestItem = (item: CustomerCartItemIdentifier) => {
...
const decreaseGuestItem = (item: CustomerCartItemIdentifier) => {
  const items = readGuestItems()
    .map((cartItem) =>
      isSameItem(cartItem, item)
        ? { ...cartItem, quantity: cartItem.quantity - 1 }
...
const removeGuestItem = (item: CustomerCartItemIdentifier) => {
